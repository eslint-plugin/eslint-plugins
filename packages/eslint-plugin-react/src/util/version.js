...
const resolve = require("@eslintplugin/resolve");
...
function convertConfVerToSemver(confVer) {
  const fullSemverString = /^[0-9]+\.[0-9]+$/.test(confVer)
    ? `${confVer}.0`
    : confVer;
...
function readDefaultReactVersionFromContext(context) {
  ...
  if (
    ...
    context.settings.react.defaultVersion
  ) {
...
function getReactVersionFromContext(context) {
  ...
  if (
    context.settings &&
    context.settings.react &&
    context.settings.react.version
  ) {
