  ...
  create(context) {
    ...
    function hasEmptyLines(first, second) {
      const comments = sourceCode.getCommentsBefore
        ? sourceCode.getCommentsBefore(second)
        : [];
    ...
    function checkSpacing(prev, node) {
              ...
              if (currNode.loc.start.line - prevNode.loc.end.line >= 2) {
                ...
                fixes.push(
                  fixer.replaceTextRange(
                    [prevNode.range[1], currNode.range[0]],
                    `\n${indent}`,
                  ),
                );
              }
      ...
      const between = getSourceCode(context).text.slice(
        prev.range[1],
        node.range[0],
      );
    ...
    function getGenericNode(node) {
      ...
      if (containsGenericType(node)) {
        ...
        return Object.assign({}, node, {
          range: [name.range[0], nodeTypeArguments.range[1]],
        });
      }
