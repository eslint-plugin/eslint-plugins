...
function hasUsedObjectDestructuringSyntax(params) {
  return (
    params != null && params.length >= 1 && params[0].type === "ObjectPattern"
  );
}
...
function verifyDefaultPropsDestructuring(context, properties) {
  ...
  properties
    .filter(
      (prop) =>
        prop.type === "Property" && prop.value.type === "AssignmentPattern",
    )
    .forEach((prop) => {
...
module.exports = {
  ...
  create: Components.detect((context, components) => ({
    "Program:exit"() {
      ...
      Object.values(list)
        .filter((component) =>
          hasUsedObjectDestructuringSyntax(component.node.params),
        )
        .forEach((component) => {
