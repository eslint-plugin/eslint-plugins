...
const messages = {
  ...
  propIsForbiddenWithValue:
    'Prop "{{prop}}" with value "{{propValue}}" is forbidden on DOM Nodes',
};
  ...
  create(context) {
    ...
    const forbid = new Map(
      (configuration.forbid || DEFAULTS).map((value) => {
        ...
        return [
          ...
          {
            disallowList:
              typeof value === "string" ? null : value.disallowedFor || null,
            disallowedValues:
              typeof value === "string" ? null : value.disallowedValues || null,
    ...
    return {
      JSXAttribute(node) {
        ...
        const isValuesListSpecified =
          forbid.get(prop).disallowedValues !== null;
        const message =
          customMessage ||
          (isValuesListSpecified && messages.propIsForbiddenWithValue) ||
          messages.propIsForbidden;
        const messageId =
          !customMessage &&
          ((isValuesListSpecified && "propIsForbiddenWithValue") ||
            "propIsForbidden");
