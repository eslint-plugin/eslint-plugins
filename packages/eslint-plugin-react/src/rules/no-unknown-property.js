...
function getDOMPropertyNames(context) {
  return [].concat(
    ...
    testReactVersion(context, ">= 16.1.0")
      ? [].concat(
          testReactVersion(context, ">= 16.4.0")
            ? [].concat(
...
function normalizeAttributeCase(name) {
  return (
    DOM_PROPERTIES_IGNORE_CASE.find(
...
function getStandardName(name, context) {
  if (DOM_ATTRIBUTE_NAMES.hasOwnProperty(name)) {
    return DOM_ATTRIBUTE_NAMES[/** @type {keyof DOM_ATTRIBUTE_NAMES} */ (name)];
  }
  if (SVGDOM_ATTRIBUTE_NAMES.hasOwnProperty(name)) {
    return SVGDOM_ATTRIBUTE_NAMES[
      /** @type {keyof SVGDOM_ATTRIBUTE_NAMES} */ (name)
    ];
  }
...
module.exports = {
  ...
  create(context) {
    function getIgnoreConfig() {
      return (
        (context.options[0] && context.options[0].ignore) || DEFAULTS.ignore
      );
    }

    function getRequireDataLowercase() {
      return context.options[0] &&
        typeof context.options[0].requireDataLowercase !== "undefined"
      ...
      JSXAttribute(node) {
        ...
        if (isValidDataAttribute(name)) {
          if (getRequireDataLowercase() && hasUpperCaseCharacter(name)) {
            report(
              context,
              messages.dataLowercaseRequired,
              "dataLowercaseRequired",
              {
                node,
                data: {
        ...
        const allowedTags = ATTRIBUTE_TAGS_MAP.hasOwnProperty(name)
        ...
        const hasStandardNameButIsNotUsed =
        ...
        if (hasStandardNameButIsNotUsed) {
          ...
          report(
            context,
            messages.unknownPropWithStandardName,
            "unknownPropWithStandardName",
            {
              node,
              data: {
