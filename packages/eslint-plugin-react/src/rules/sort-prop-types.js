  ...
  create(context) {
    ...
    function checkSorted(declarations) {
      ...
      declarations.reduce((prev, curr, idx, decls) => {
        ...
        const previousIsCallback =
          propTypesSortUtil.isCallbackPropName(prevPropName);
        const currentIsCallback =
          propTypesSortUtil.isCallbackPropName(currentPropName);
    ...
    function handleFunctionComponent(node) {
      ...
      if (firstArg && firstArg.type === "TSTypeReference") {
        ...
        const propType =
          typeAnnotations.get(firstArg.id.name) &&
