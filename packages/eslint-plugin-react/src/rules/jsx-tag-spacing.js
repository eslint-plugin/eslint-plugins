  ...
  if (node.selfClosing) {
    ...
    if (option === "never") {
      if (!adjacent) {
        report(
          context,
          messages.selfCloseSlashNoSpace,
          "selfCloseSlashNoSpace",
          {
            node,
            loc: {
            ...
            fix(fixer) {
              return fixer.removeRange([
                lastTokens[0].range[1],
                lastTokens[1].range[0],
              ]);
            },
          },
        );
      ...
      report(
        context,
        messages.selfCloseSlashNeedSpace,
        "selfCloseSlashNeedSpace",
        {
          node,
          loc: {
    ...
    if (option === "never") {
      if (!adjacent) {
        report(context, messages.closeSlashNoSpace, "closeSlashNoSpace", {
          ...
          fix(fixer) {
            return fixer.removeRange([
              firstTokens[0].range[1],
              firstTokens[1].range[0],
            ]);
          },
        });
      }
...
function validateBeforeSelfClosing(context, node, option) {
  ...
  if ((option === "always" || option === "proportional-always") && adjacent) {
    report(
      context,
      messages.beforeSelfCloseNeedSpace,
...
function validateBeforeClosing(context, node, option) {
  ...
  if (!node.selfClosing) {
    ...
    if (option === "never" && !adjacent) {
      ...
      adjacent !== (node.loc.start.line === node.loc.end.line)
    ) {
      report(context, messages.beforeCloseNeedSpace, "beforeCloseNeedSpace", {
        node,
        loc: {
