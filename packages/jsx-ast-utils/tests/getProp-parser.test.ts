...
import { getOpeningElement, setParserName, type ParserName } from "./helper";
...
function actualTest(parserName: ParserName, test) {
  ...
  expect(adjustLocations(sourceResult, offset)).toStrictEqual(
...
function stripConstructors<T>(value: T) {
  return JSON.parse(JSON.stringify(value));
}

function adjustLocations(node: JSXAttribute, { keyOffset, valueOffset }) {
