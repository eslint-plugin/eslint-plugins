...
const realpathFS =
  process.platform !== "win32" ? fs.realpath.native : fs.realpath;
...
const defaultRealpath = function realpath(x, cb) {
  ...
  isFile(pkgfile, function (err, exists) {
    ...
    if (exists) {
      readPackage(readFile, pkgfile, function (readErr, pkg) {
        if (readErr && !(readErr instanceof SyntaxError)) {
          return done(readErr);
        }
        done(null, pkg || null);
      });
...
export default function resolve(x, options, callback) {
  let cb = callback;
  let opts = options;
  ...
  const readPackage = opts.readPackage || defaultReadPackage;
  if (opts.readFile && opts.readPackage) {
    const conflictErr = new TypeError(
  ...
  const absoluteStart = path.resolve(basedir);
  ...
  function loadAsFile(x, thePackage, callback) {
    let loadAsFilePackage = thePackage;
    let cb = callback;
    ...
    function load(exts, x, loadPackage) {
      ...
      function onpkg(err, pkg_, dir) {
        ...
        if (dir && pkg && opts.pathFilter) {
          const rfile = path.relative(dir, file);
          const rel = rfile.slice(0, rfile.length - exts[0].length);
          const r = opts.pathFilter(pkg, x, rel);
          if (r)
            return load([""].concat(extensions), path.resolve(dir, r), pkg);
        }
        isFile(file, onex);
      }
  ...
  function loadAsDirectory(x, loadAsDirectoryPackage, callback) {
    let cb = callback;
    let fpkg = loadAsDirectoryPackage;
        ...
        readPackage(readFile, pkgfile, function (err, pkgParam) {
          ...
          if (pkg && pkg.main) {
            ...
            loadAsFile(path.resolve(x, pkg.main), pkg, function (err, m, pkg) {
              ...
              const dir = path.resolve(x, pkg.main);
